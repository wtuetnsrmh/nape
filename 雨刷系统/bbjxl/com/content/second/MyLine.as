package bbjxl.com.content.second{
	...
	**/
	...
	import flash.events.*;
	public class MyLine extends Sprite {
		...
		private var pointRadius:uint=4;//点的半径
		...
		public function hitPin():void{
			...
			this.rotation=30;
			bs.gotoAndStop(2);
			TailPoint.x=24;//Math.pow(Math.sin(rota),2)*Math.abs(y2-y1)*Math.tan(rota);
			
			TailPoint.y=-28;//y1-TailPoint.x*Math.tan(rota)+y2;
			/*TailPoint.x=-2.05;//Math.pow(Math.sin(rota),2)*Math.abs(y2-y1)*Math.tan(rota);
			TailPoint.y=-39;//y1-TailPoint.x*Math.tan(rota)+y2;*/
			}
		//笔头没接触到
