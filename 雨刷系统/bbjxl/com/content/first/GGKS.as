package bbjxl.com.content.first
{
	...
	**/
	...
	public class GGKS extends Sprite
	{
		
		
		/*[Embed(source="dataImages/alertBg.png")]
		public var $alertBg:Class*/
		...
		private var tryEndShowScore:GGKS_ShowScore;//显示得分

		//数据
		private var mnksWs:WebService=new WebService(Gvar.WebServerUrl);
		private	var op:Operation = new Operation(mnksWs);
		...
		private function checkExam():void {
			...
			op.PartExamValid( { UserId:Gvar.getInstance().UserId, CoursewareId:Gvar.getInstance().CoursewareId } );
			op.addEventListener(OperationEvent.COMPLETE, checkExamWscomplete);
		...
		//
		private function checkExamWscomplete(e:OperationEvent):void {
		...
		{
			op.PartExam({ CoursewareId:Gvar.getInstance().CoursewareId});
			op.addEventListener(OperationEvent.COMPLETE, mnksWscomplete);
			op.addEventListener(OperationEvent.FAILED, onFault);
			
		}
		
	//过关考试
		private function mnksWscomplete(e:OperationEvent):void
			{
				//trace(e.data);
				tryArr=new Array();
				tryXml=XML(e.data);
				...
				Gvar.getInstance().Examid = _examid;//ID存储
		...
		private function bottonClickEvent(e:MNKS_Botton_PageClickEvent):void{
			...
			//trace("_currentSelectId="+_currentSelectId)
		...
		private function judgAndShowScore():void{
			tryEndShowScore=new GGKS_ShowScore(allTiAndSelected);
		...
		private function creaBg()
		{
			...
			//建立标题
			var tempTitle:CreaText=new CreaText("部件认识--过关考试",0xffffff,16,true);
